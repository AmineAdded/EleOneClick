            ...
            <div class="page-header pa-6">
                <div class="header-content">
                    <div class="title-container">
                        <h1 class="text-h3 font-weight-bold gradient-text mb-2">Gestion des taches</h1>
                        <div class="subtitle text-subtitle-1 text-medium-emphasis mb-4">
                            Gérez efficacement les taches de votre équipe
                        </div>
                    </div>
                ...
                <v-card class="rounded-lg">
                    ...
                    <v-card-text class="pa-6 pt-8">
                        <div class="d-flex align-center mb-4">
                            ...
                            <div>
                                <div class="text-h6 mb-1">Supprimer la tache</div>
                                <div class="text-body-1 text-medium-emphasis">
                                    Vous êtes sur le point de supprimer les informations de la tache :
            ...
            <v-card-title class="table-header pa-6">
                <div class="d-flex flex-wrap align-center justify-space-between gap-4">
                    <div class=" pa-4 rounded" style="position: relative;top: 10px;">
                        <AjoutTache @task-added="refreshTasks" @showMessage="handleMessage" />
                    </div>
                    <v-text-field v-model="search" prepend-icon="mdi-magnify" placeholder="Rechercher une tache..."
                        single-line hide-details class="custom-search-field" variant="outlined" density="compact">
                    </v-text-field>
                </div>
            </v-card-title>

            <v-data-table :headers="headers" :items="paginatedTasks" :search="search" :items-per-page="itemsPerPage"
                ...
                <template v-slot:item="{ item }">
                    <tr>
                        ...
                        <td>
                            <div class="font-weight-medium">{{ item.description === null ? 'description indisponible' :
                ...
                <template v-slot:bottom>
                    <div class="d-flex align-center justify-space-between pa-4">
                        <div class="text-caption text-medium-emphasis">
                            {{ filteredPersonnels.length }} taches au total
                        </div>
...
<script>
import AjoutTache from "@/components/Formateur/AjoutTache.vue";
...
export default {
    ...
    computed: {
        filteredPersonnels() {
            return this.tasks.filter(task =>
                ...
                String(task.uap_id).includes(this.search) ||
                String(task.id).includes(this.search)
            );
        },
    ...
    methods: {
        ...
        async confirmDelete() {
            try {
                await TaskService.deleteTask(this.itemToDelete.id);
        ...
        async showTasks() {
            try {
                const data = await TaskService.showTasks();
